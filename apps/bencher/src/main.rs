...
fn main() {
    ...
    unsafe {
     ...
     clk_sel, clk_frac, sclk_sel, pclk_en,  sclk_en);
        ...
        //core::ptr::write_volatile((cru_base + 0x0834 ) as *mut u32, sclk_en | 0xe | (0xe << 16)); // CRU_GATE_CON13, SCLK_UART7, sclk_uart7_en bit15
        core::ptr::write_volatile(
    ...
    for i in 0..iter {
        ...
        let cpu_freq = cycle::cpu_freq(cpu_cycle, timer_sum);
        println!(
            "\nIn {}s, {}ns, Average cost CPU cycles: {}",
            s_sum,
            ns_sum,
            cpu_cycle / switch_count
        );
        println!(
            "CPU Freq = {}Hz, CPU Cycle Counter = {} from {} to {}",
            cpu_freq, cpu_cycle, cpu_cycle_start, cpu_cycle_end
        );
    }
